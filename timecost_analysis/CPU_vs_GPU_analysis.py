...
def main_inference():
    ...
    for datasetName in tqdm(datasetInfo.keys(), desc='vSTMD_cpu'):
        ...
        time_spend_dict['vSTMD_cpu'] += _task_vSTMD_cpu(inputPath)

    for datasetName in tqdm(datasetInfo.keys(), desc='vSTMD_gpu'):
        # Dataset path
        inputPath = os.path.join(ristDatasetPath, datasetName, f'{datasetName}.mp4')
        time_spend_dict['vSTMD_gpu'] += _task_vSTMD_gpu(inputPath)

    for datasetName in tqdm(datasetInfo.keys(), desc='vSTMD_F_cpu'):
        # Dataset path
        inputPath = os.path.join(ristDatasetPath, datasetName, f'{datasetName}.mp4')
        time_spend_dict['vSTMD_F_cpu'] += _task_vSTMD_F_cpu(inputPath)

    for datasetName in tqdm(datasetInfo.keys(), desc='vSTMD_F_gpu'):
        # Dataset path
        inputPath = os.path.join(ristDatasetPath, datasetName, f'{datasetName}.mp4')
        time_spend_dict['vSTMD_F_gpu'] += _task_vSTMD_F_gpu(inputPath)



    for key in time_spend_dict.keys():
        time_spend_dict[key] /= len(datasetInfo.keys())  # average time cost per frame
